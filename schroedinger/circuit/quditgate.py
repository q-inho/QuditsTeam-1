...
class QuditGate(QuditInstruction):
    ...
    def __init__(self, name: str, qudit_dimensions: List[int], num_single_qubits: int, params: List,
        ...
        # calls __init__ of QuditInstruction (and Instruction), but not Gate
        super().__init__(name, qudit_dimensions, num_qubits, 0, params)
    ...
    def to_matrix(self) -> np.ndarray:
        ...
        Returns:
            np.ndarray: if the QuditGate subclass has a matrix definition.

        Raises:
            CircuitError: If a QuditGate subclass does not implement this method an
    ...
    def _return_repeat(self, exponent: float) -> 'QuditGate':
        return QuditGate(name="%s*%s" % (self.name, exponent),
